...
dat <- read.csv('data/tidy/sea_final_choice.csv',stringsAsFactors = F)
...
dat %<>% 
  group_by(rn = rownames(dat)) %>% 
  mutate(scent = func1(c(A, B)))

tmp <- data.frame(scent=unique(dat$scent))
# tmp$choice <- c('Bnk', 'Ad', 'Ju', 'Ad', 'Bnk', 'Bnk' )    # you don't need to change this - you already have a coded column called choice.
...
dat$tankside <- ifelse(dat$A == dat$choice, 
                       paste('A', dat$tank, sep = ''), 
                       paste('B', dat$tank, sep=''))
head(dat)

scent <- c('Blank',  'Seagrass', 'Lobster', 'Mix')

pin   <- c('B' = -3, 'S' = -0.7, 'L' = -3, 'M' = -1.5, 
...
# remove rows where lobsters didn't make a choice
...
mod <- function(x, flag = 'solve'){
  ...
  dat$res     <- ifelse(dat$A == dat$choice, 
                        dat$tA/(dat$tA+dat$tB), 
                        dat$tB / (dat$tA + dat$tB)) + dat$tankEff
...
legend('bottom',ncol=4,fill=Col,legend=(rownames(tmp)),inset = -0.85)
...
legend('bottom',ncol=4,fill=Col,legend=names(pref),inset = -0.80)
...
##Bootstrap
odat    <- dat 
bstrap  <- 1000 ##x100 in Michael's
Prefout <- array(NA, dim = c(4, 4, bstrap), 
                 dimnames = list(a = names(pin)[1:4], b = names(pin)[1:4], ob = 1:bstrap))
Tankout <- matrix(NA, nrow = bstrap, ncol = length(pin) - 4, 
                  dimnames = list(ob = 1:bstrap, par = names(pin)[5:length(pin)]))
...
bp <- barplot(Prefout50, beside = T, col = Col, 
              ylim=c(-0.5,0.5), 
              names.arg = paste(scent,'vs ..'), 
              ylab = 'Relative preference', xlab = 'Scent Options')
...
bp       <- barplot(Tankout2['50%',], beside = T, 
                    ylab = 'Tank effect from A1',
                    ylim = c(-mx, mx), xlab = 'Tank side and number')
...
col.order   <- c("Mix", "Seagrass", "Blank", "Lobster")
...
Scent_Data <- Scent_Means %>%
  full_join(Scent_Upper, by = "Comparison")%>%
  full_join(Scent_Lower, by = "Comparison")%>%
  glimpse()
...
scent <- c('Mix','Seagrass','Blank','Lobster')
...
m = c(1.1,1.1,0.5,1.5)
par(mfrow = c(1, 1), mar = c(5, 5, 2, 2), mai = m, las = 1)
bp <- barplot(Pref_data[c(1:4), c(1:3)], beside = T, col = Col, 
              ylim = c(-0.5,0.5), names.arg = paste(scent[1:3], 'vs ..'), 
              ylab = 'Frequency chosen', xlab = 'Scent Options')
lines(x_coord[1:2], y_coord[1:2], col = colours[1], type = "l", lwd = 2)
lines(x_coord[3:4], y_coord[3:4], col = colours[2], type = "l", lwd = 2)
lines(x_coord[5:6], y_coord[5:6], col = colours[3], type = "l", lwd = 2)
#lines(x_coord[7:8],y_coord[7:8],col= colours[4], type="l", lwd=2)
arrows(bp, Pref_lower[c(1:4), c(1:3)],
       y1 = Pref_upper[c(1:4), c(1:3)],
